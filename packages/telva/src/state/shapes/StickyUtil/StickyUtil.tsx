...
export class StickyUtil extends TVShapeUtil<T, E> {
  type = TVShapeType.Sticky as const
  ...
  Component = TVShapeUtil.Component<T, E, TVMeta>(
    ({ shape, meta, events, isGhost, isBinding, isEditing, onShapeBlur, onShapeChange }, ref) => {
      ...
      const { color, fill, stroke } = getStickyShapeStyle(shape.style, meta.isDarkMode)
      ...
      const textGradient = shape.style.textGradient
      const style = {
        font,
  ...
  getSvgElement = (shape: T, isDarkMode: boolean): SVGElement | void => {
    ...
    const fontFamily = getFontFace(shape.style.font, shape.style.fontFamily).slice(1, -1)
