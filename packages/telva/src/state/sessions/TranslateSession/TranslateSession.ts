import { TLBounds, TLBoundsWithCenter, TLPageState, TLSnapLine, Utils } from 'telva-core'
...
export class TranslateSession extends BaseSession {
  ...
  initialShapes: TVShape[]
  initialParentChildren: Record<string, string[]>
  bindingsToDelete: ArrowBinding[]

  constructor(app: TelvaApp, isCreate = false, link: 'left' | 'right' | 'center' | false = false) {
    super(app)
    ...
    const selectedShapes = (
      link ? TVDR.getLinkedShapeIds(this.app.state, currentPageId, link, false) : selectedIds
    )
    ...
    this.initialCommonBounds = Utils.getCommonBounds(this.initialShapes.map(TVDR.getRotatedBounds))

    this.app.rotationInfo.selectedIds = [...this.app.selectedIds]
  }

  start = (): TelvaPatch | undefined => {
    const {
      bindingsToDelete,
      initialIds,
    ...
    Object.values(page.shapes).forEach((shape) => {
      const bounds = Utils.getBoundsWithCenter(TVDR.getRotatedBounds(shape))
    ...
    const nextBindings: Patch<Record<string, TVBinding>> = {}
    const nextShapes: Patch<Record<string, TVShape>> = {}
  ...
  update = (): TelvaPatch | undefined => {
    ...
    if (this.isCloning) {
      ...
      if (didChangeCloning) {
        ...
        initialShapes.forEach((shape) => {
          // const current = (nextShapes[shape.id] || this.app.getShape(shape.id)) as TVShape
  ...
  cancel = (): TelvaPatch | undefined => {
    ...
    const nextBindings: Record<string, Partial<TVBinding> | undefined> = {}
    const nextShapes: Record<string, Partial<TVShape> | undefined> = {}
  ...
  complete = (): TelvaPatch | TelvaCommand | undefined => {
    ...
    const beforeBindings: Patch<Record<string, TVBinding>> = {}
    const beforeShapes: Patch<Record<string, TVShape>> = {}

    const afterBindings: Patch<Record<string, TVBinding>> = {}
    const afterShapes: Patch<Record<string, TVShape>> = {}
  ...
  private createCloneInfo = () => {
    ...
    const clonedBindings: TVBinding[] = []

    // Create clones of selected shapes
    const clones: TVShape[] = []
    ...
    initialShapes.forEach((shape) => {
      ...
      const clone = {
        ...
        childIndex: TVDR.getChildIndexAbove(this.app.state, shape.id, currentPageId),
      }

      if (clone.type === TVShapeType.Video) {
