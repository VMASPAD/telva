...
import { AlignType, DistributeType, StretchType, TVExportType, TVSnapshot } from '~types'

const numberOfSelectedIdsSelector = (s: TVSnapshot) => {
  return s.document.pageStates[s.appState.currentPageId].selectedIds.length
}

const isDebugModeSelector = (s: TVSnapshot) => {
  return s.settings.isDebugMode
}

const hasGroupSelectedSelector = (s: TVSnapshot) => {
...
const InnerMenu = React.memo(function InnerMenu() {
  ...
  const handleCopySVG = React.useCallback(() => {
    app.copyImage(TVExportType.SVG, { scale: 1, quality: 1, transparentBackground: false })
  }, [app])

  const handleCopyPNG = React.useCallback(() => {
    app.copyImage(TVExportType.PNG, { scale: 2, quality: 1, transparentBackground: true })
  }, [app])
  ...
  const handleExportWEBP = React.useCallback(async () => {
    app.exportImage(TVExportType.WEBP, { scale: 2, quality: 1 })
  }, [app])
  ...
  const selectedShape =
    numberOfSelectedIds === 1 ? app.getShape(app.selectedIds[0], app.currentPageId) : null
  const isReactComponent = selectedShape?.type === 'reactComponent'
    ...
    <>
      {hasSelection ? (
        <>
          ...
          {isReactComponent && (
            <>
              <CMRowButton onClick={handleConvertToImage} id="TD-ContextMenu-Convert-To-Image">
                Convert to Image
              </CMRowButton>
              <Divider />
            </>
...
const currentPageIdSelector = (s: TVSnapshot) => s.appState.currentPageId
const documentPagesSelector = (s: TVSnapshot) => s.document.pages
