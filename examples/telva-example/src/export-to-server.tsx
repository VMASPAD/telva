...
export default function Export() {
  const handleExport = React.useCallback(async (app: TelvaApp) => {
    exportViaServer(app, TVExportType.PNG)
  }, [])
...
async function exportViaServer(app: TelvaApp, type: TVExportType) {
  ...
  try {
    const assets: TVAssets = {}

    // Collect shapes and assets (serializing assets if needed)
    const shapes: TVShape[] = shapeIds.map((id) => {
      ...
      if (shape.assetId) {
        ...
        if (shape.type === TVShapeType.Video) {
          asset.src = app.serializeVideo(shape.id)
          asset.type = TVAssetType.Image
          // Cast shape to image shapes to properly display snapshots
          ;(shape as unknown as ImageShape).type = TVShapeType.Image
        }
    ...
    const { width, height } = Utils.expandBounds(TVDR.getSelectedBounds(app.state), 64)
    ...
    switch (type) {
      case TVExportType.SVG: {
        ...
        serialized = TVDR.getSvgString(svg, 1)
        break
      }
      case TVExportType.JSON: {
