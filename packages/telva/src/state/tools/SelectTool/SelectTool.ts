...
export class SelectTool extends BaseTool<Status> {
  ...
  clonePaint = (point: number[]) => {
    ...
    const bounds = Utils.expandBounds(Utils.getCommonBounds(shapes.map(TVDR.getBounds)), 16)
    ...
    const hit = this.app.shapes.some((shape) =>
      TVDR.getShapeUtil(shape).hitTestBounds(shape, centeredBounds)
    )
  ...
  getShapeClone = (
    ...
    if (utils.canClone) {
      ...
      if (clone.type === TVShapeType.Sticky) {
        clone.text = ''
      }
      ...
      case 'Tab': {
        ...
        if (
          ...
          if (clonedShape) {
            ...
            this.setStatus(Status.Idle)
            if (clonedShape.type === TVShapeType.Sticky) {
        ...
        if (Vec.dist(originPoint, currentPoint) > DEAD_ZONE) {
          if (this.pointedBoundsHandle === 'rotate') {
            ...
            const idsToTransform = this.app.selectedIds.flatMap((id) =>
              TVDR.getDocumentBranch(this.app.state, id, this.app.currentPageId)
            )
  ...
  onDoubleClickCanvas: TLCanvasEventHandler = () => {
    ...
    // Needs debugging
    // const { currentPoint } = this.app
    // this.app.selectTool(TVShapeType.Text)
    // this.setStatus(Status.Idle)
  ...
  onDoubleClickShape: TLPointerEventHandler = (info) => {
    ...
    // start editing
    if (
      TVDR.getShapeUtil(shape.type).canEdit &&
