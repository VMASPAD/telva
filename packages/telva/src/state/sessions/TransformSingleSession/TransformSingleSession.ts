...
export class TransformSingleSession extends BaseSession {
  ...
  constructor(
    ...
    this.initialShapeBounds = TVDR.getBounds(shape)
    this.initialCommonBounds = TVDR.getRotatedBounds(shape)
  ...
  update = (): TelvaPatch | undefined => {
    ...
    const shapes = {} as Record<string, Partial<TVShape>>
    ...
    const afterShape = TVDR.getShapeUtil(shape).transformSingle(shape, newBounds, {
      initialShape,
  ...
  complete = (): TelvaPatch | TelvaCommand | undefined => {
    ...
    const beforeShapes = {} as Record<string, Partial<TVShape> | undefined>
    const afterShapes = {} as Record<string, Partial<TVShape>>

    beforeShapes[initialShape.id] = this.isCreate ? undefined : initialShape

    afterShapes[initialShape.id] = TVDR.onSessionComplete(this.app.getShape(initialShape.id))
