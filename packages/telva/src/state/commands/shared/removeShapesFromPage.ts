...
import type { ArrowShape, GroupShape, PagePartial, TVSnapshot } from '~types'

export function removeShapesFromPage(data: TVSnapshot, ids: string[], pageId: string) {
  ...
  ids
    .filter((id) => !TVDR.getShape(data, id, pageId).isLocked)
    .forEach((id) => {
      deletedIds.add(id)
      ...
      if (shape.children !== undefined) {
        shape.children.forEach((childId) => {
          ...
          const child = TVDR.getShape(data, childId, pageId)
      ...
      if (shape.parentId !== pageId) {
        parentsToUpdate.push(TVDR.getShape(data, shape.parentId, pageId))
      }
  ...
  parentsToUpdate.forEach((parent) => {
    ...
    if (after.shapes[parent.id]?.children!.length === 0) {
      ...
      before.shapes[parent.id] = TVDR.getShape(data, parent.id, pageId)
    }
  })
