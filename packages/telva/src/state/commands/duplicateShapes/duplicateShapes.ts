...
import type { PagePartial, TVShape, TelvaCommand } from '~types'

export function duplicateShapes(app: TelvaApp, ids: string[], point?: number[]): TelvaCommand {
  ...
  shapesToDuplicate.forEach((shape) => {
    ...
    after.shapes[duplicatedId] = {
      ...
      id: duplicatedId,
      childIndex: TVDR.getChildIndexAbove(app.state, shape.id, currentPageId),
    }
    ...
    if (shape.children) {
      shape.children.forEach((childId) => {
        ...
        after.shapes[duplicatedId] = {
          ...
          childIndex: TVDR.getChildIndexAbove(app.state, child.id, currentPageId),
        }
  ...
  if (point) {
    const commonBounds = Utils.getCommonBounds(shapesToMove.map((shape) => TVDR.getBounds(shape)))
