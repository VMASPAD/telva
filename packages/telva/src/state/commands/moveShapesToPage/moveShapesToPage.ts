import { TLBounds, Utils } from 'telva-core'
import { Vec } from 'telva-vec'
import { TVDR } from '~state/TVDR'
import type { TelvaApp } from '~state/TelvaApp'
import type { ArrowShape, PagePartial, TVShape, TelvaCommand } from '~types'
...
export function moveShapesToPage(
  ...
  const shapesToMove = new Set<TVShape>()
  ...
  const startingChildIndex = TVDR.getTopChildIndex(app.state, toPageId)
  ...
  const bounds = Utils.getCommonBounds(movingShapes.map((shape) => TVDR.getBounds(shape)))

  const zoom = TVDR.getCameraZoom(
