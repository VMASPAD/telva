...
function _Canvas<T extends TLShape, M extends Record<string, unknown>>({
  ...
  return (
    <div id={id} className="tv-container" ref={rContainer}>
      <div id="canvas" className="tv-absolute tv-canvas" ref={rCanvas} {...events}>
        {!hideGrid && grid && <Grid grid={grid} camera={pageState.camera} />}
        <div ref={rLayer} className="tv-absolute tv-layer" data-testid="layer">
          <Page
            page={page}
            pageState={pageState}
            assets={assets}
