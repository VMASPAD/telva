...
import { TVExportType, TVSnapshot } from '~types'
...
const disableAssetsSelector = (s: TVSnapshot) => {
...
export const Menu = React.memo(function Menu({ readOnly }: MenuProps) {
  ...
  const handleSaveProjectAs = React.useCallback(() => {
    // Always open the dialog so the user can choose filename + format (.tldr / .tld)
    setOpenDialog(true)
  }, [])
