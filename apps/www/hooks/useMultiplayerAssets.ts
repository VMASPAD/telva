...
import { TVAsset, TelvaApp } from 'telva'
...
export function useMultiplayerAssets() {
  const onAssetCreate = useCallback(
    ...
    async (app: TelvaApp, file: File, id: string): Promise<string | false> => {
  ...
  const onAssetDelete = useCallback(async (app: TelvaApp, id: string): Promise<boolean> => {
    // noop
    return true
  }, [])
