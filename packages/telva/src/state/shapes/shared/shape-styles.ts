...
export function getFontFace(font: FontStyle = FontStyle.Script, fontFamily?: string): string {
...
export function getFontStyle(style: ShapeStyles): string {
  ...
  const fontFace = getFontFace(style.font, style.fontFamily)
  const { scale = 1 } = style
...
export function getStickyShapeStyle(style: ShapeStyles, isDarkMode = false) {
  ...
  return {
    fill: style.fillHex ?? stickyFills[theme][adjustedColor],
    stroke: style.strokeHex ?? null,
...
export function getShapeStyle(
  ...
  const strokeWidth = style.strokeWidthOverride ?? getStrokeWidth(size)

  const theme: Theme = isDarkMode ? 'dark' : 'light'

  return {
    stroke: style.strokeHex ?? strokes[theme][color],
    fill: isFilled ? style.fillHex ?? fills[theme][color] : 'none',
    strokeWidth,
