...
import { ArrowShape, ColorStyle, SessionType, TVShapeType } from '~types'
...
describe('TelvaTestApp', () => {
  describe('When copying and pasting...', () => {
    it('copies a shape', () => {
      new TelvaTestApp().loadDocument(mockDocument).selectNone().copy(['rect1'])
    })

    it('pastes a shape', () => {
      const app = new TelvaTestApp().loadDocument(mockDocument)
    ...
    it('Copies grouped shapes.', () => {
      ...
      expect(app.shapes.filter((shape) => shape.type === TVShapeType.Group).length).toBe(2)
      ...
      const newGroup = newShapes.find((shape) => shape.type === TVShapeType.Group)
  ...
  describe('When copying and pasting a shape with bindings', () => {
    it('copies two bound shapes and their binding', () => {
      ...
      app
        .createShapes(
          { type: TVShapeType.Rectangle, id: 'target1', point: [0, 0], size: [100, 100] },
          { type: TVShapeType.Arrow, id: 'arrow1', point: [200, 200] }
        )
        .select('arrow1')
        .movePointer([200, 200])
  ...
  describe('Selection', () => {
    it('selects a shape', () => {
      const app = new TelvaTestApp().loadDocument(mockDocument).selectNone().clickShape('rect1')
    ...
    describe('When selecting all', () => {
      it('selects all', () => {
        const app = new TelvaTestApp().loadDocument(mockDocument).selectAll()
        expect(app.selectedIds).toMatchSnapshot('selected all')
      })

      it('does not select children of a group', () => {
        const app = new TelvaTestApp().loadDocument(mockDocument).selectAll().group()
  ...
  describe('when creating shapes', () => {
    it('Creates shapes with the correct child index', () => {
      const app = new TelvaTestApp()
        ...
        .selectTool(TVShapeType.Rectangle)
      ...
      app.selectNone()
      app.selectTool(TVShapeType.Rectangle)
  ...
  it('Sets the undo/redo history', () => {
    const app = new TelvaTestApp('some_state_a')
      .createShapes({
        id: 'rect1',
        type: TVShapeType.Rectangle,
        point: [0, 0],
    ...
    // Create a new state
    const state2 = new TelvaTestApp('some_state_b')
  ...
  describe('When copying to SVG', () => {
    it('Copies shapes.', async () => {
      const result = await new TelvaTestApp()
        .loadDocument(mockDocument)
        .select('rect1')
        .rotate(0.1)
        .selectAll()
  ...
  /*
    ...
    When the `document` prop changes in the Telva component, we want
  ...
  */
  ...
  describe('When changing versions', () => {
    it('migrates correctly', async () => {
      const defaultState = TelvaTestApp.defaultState
      const withoutRoom = {
        ...defaultState,
      }
      delete withoutRoom.room
      TelvaTestApp.defaultState = withoutRoom
      const app = new TelvaTestApp('migrate_1')
      await app.ready
      app.createShapes({
        id: 'rect1',
        type: TVShapeType.Rectangle,
      })
      TelvaTestApp.version = 100
      TelvaTestApp.defaultState.room = defaultState.room
      const app2 = new TelvaTestApp('migrate_1')
      await app2.ready
  ...
  describe('When replacing the page content', () => {
    it('Should update the page with the correct shapes and bindings.', async () => {
      ...
      const app = new TelvaTestApp('multiplayer', {
        onChangePage: () => {
          //
        },
      })
  ...
  describe('When selecting a box', () => {
    const app = new TelvaTestApp()
    app
      .createShapes({ id: 'box1', type: TVShapeType.Rectangle, point: [0, 0], size: [100, 100] })
      .pointCanvas([-50, 20])
      .movePointer([50, 50])
      .movePointer([50, 51])
...
describe('Arrow binding', () => {
  it('should delete handles bindingId', () => {
    ...
    app
      .createShapes(
        { type: TVShapeType.Rectangle, id: 'target1', point: [0, 0], size: [100, 100] },
        { type: TVShapeType.Rectangle, id: 'target2', point: [300, 300], size: [100, 100] },
        { type: TVShapeType.Arrow, id: 'arrow1', point: [200, 200] }
      )
      .select('arrow1')
      .movePointer([200, 200])
...
describe('When using a pen eraser', () => {
  it('switches to eraser tool when using the draw tool and back when letting go', () => {
    const app = new TelvaTestApp()
    app.selectTool(TVShapeType.Draw)
    app.pressEraser()
    expect(app.currentTool.type).toBe('erase')
    app.releaseEraser()
    expect(app.currentTool.type).toBe(TVShapeType.Draw)
  })
