...
import { TVDockPosition, TVExportBackground, TVSnapshot } from '~types'

const settingsSelector = (s: TVSnapshot) => s.settings
  ...
  return (
    <DMSubMenu label={intl.formatMessage({ id: 'menu.preferences' })} id="TD-MenuItem-Preferences">
      ...
      <DMSubMenu label={intl.formatMessage({ id: 'dock.position' })}>
        {DockPosition.map((position) => (
          <DMCheckboxItem
            ...
            onCheckedChange={() => handleChangeDockPosition(position as TVDockPosition)}
      ...
      <DMSubMenu label={intl.formatMessage({ id: 'export.background' })}>
        {Object.values(TVExportBackground).map((exportBackground) => (
          <DMCheckboxItem
            key={exportBackground}
            checked={settings.exportBackground === exportBackground}
            onCheckedChange={() => selectExportBackground(exportBackground as TVExportBackground)}
