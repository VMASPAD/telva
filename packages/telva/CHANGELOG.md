...
- - Adds `components` prop to Telva component (for custom Cursor, etc) @jamesbvaughan
...
- - Remove `mobx` and `mobx-react-lite` as dependencies. This is a breaking change for libraries that expect data to be observable in `@telva/core`.
...
- Fixes zooming and pinching bugs. Adds ErrorBoundary to Telva component. Cleans up sponosrship feature in menu.
...
- Prevent clipboard events inside of text from reaching the document. Fix various clipboard-related bugs in telva.
- Updated dependencies
  - @telva/core@1.11.0
...
- Fix telva assets for vscode extension.

## 1.12.0

### Minor Changes

- This update changes how clipboard actions (cut, copy, paste) and exports work. Significantly, image exports are no longer handled via a server-side integration, and are instead handled locally on the client. This allows now for exports in the VS Code extension, as well as greatly simplifying exports for apps that embed the Telva React component.
...
- Support for assets (images and videos) in telva multiplayer.
- Support for image exporting in telva.
...
- Renames TLDraw to telva throughout the app and documentation.
- Renames TLDrawState to TelvaApp, state to app.
...
- Extracted into its own repository (`telva/core`) and open sourced! :clap:

### telva

- Updated with latest `@telva/core`, updated ShapeUtils API.
...
- Adds [side cloning](https://github.com/telva/telva/pull/149).
- Improves rendering.

### telva

- Adds sticky note [side cloning](https://github.com/telva/telva/pull/149).
...
- Fixes centering on embedded telva components.
