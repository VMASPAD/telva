...
import { SessionArgsOfType, TelvaSession, getSession } from './sessions'
...
export class TelvaApp extends StateManager<TVSnapshot> {
  callbacks: TVCallbacks = {}

  tools = {
    select: new SelectTool(this),
    erase: new EraseTool(this),
    [TVShapeType.Text]: new TextTool(this),
    [TVShapeType.Draw]: new DrawTool(this),
    [TVShapeType.Ellipse]: new EllipseTool(this),
    [TVShapeType.Rectangle]: new RectangleTool(this),
    [TVShapeType.Triangle]: new TriangleTool(this),
    [TVShapeType.Line]: new LineTool(this),
    [TVShapeType.Arrow]: new ArrowTool(this),
    [TVShapeType.Sticky]: new StickyTool(this),
    [TVShapeType.Pen]: new PenTool(this),
  }
  ...
  constructor(id?: string, callbacks = {} as TVCallbacks) {
    super(TelvaApp.defaultState, id, TelvaApp.version, (prev, next, prevVersion) => {
      return migrate(
        {
          ...next,
  ...
  protected migrate = (state: TVSnapshot): TVSnapshot => {
    return migrate(state, TelvaApp.version)
  }
  ...
  protected onReady = () => {
    ...
    try {
      this.patchState({
        ...migrate(this.state, TelvaApp.version),
        appState: {
      ...
      this.patchState({
        ...TelvaApp.defaultState,
        appState: {
          ...TelvaApp.defaultState.appState,
          status: TVStatus.Idle,
        },
      })
    }
  ...
  protected cleanup = (state: TVSnapshot, prev: TVSnapshot): TVSnapshot => {
    ...
    if (next.document !== prev.document) {
      Object.entries(next.document.pages).forEach(([pageId, page]) => {
        ...
        const changedShapes: Record<string, TVShape | undefined> = {}
        ...
        if (!prevPage || page.shapes !== prevPage.shapes || page.bindings !== prevPage.bindings) {
          ...
          const bindingsToUpdate = TVDR.getRelatedBindings(next, Object.keys(changedShapes), pageId)
          ...
          bindingsToUpdate.forEach((binding) => {
            ...
            const fromDelta = TVDR.updateArrowBindings(page, fromShape)
          ...
          groupsToUpdate.forEach((group) => {
            ...
            const commonBounds = Utils.getCommonBounds(
              children
                ...
                .filter(Boolean)
                .map((shape) => TVDR.getRotatedBounds(shape))
            )
    ...
    if (next.room) {
      const roomUser = next.room.users[next.room.userId]

      next.room.users[next.room.userId] = {
        ...roomUser,
        id: roomUser?.id ?? next.room.userId,
        color: roomUser?.color ?? USER_COLORS[0],
        activeShapes: roomUser?.activeShapes ?? [],
        status: roomUser?.status ?? TVUserStatus.Connected,
  ...
  private broadcastPatch = (patch: TelvaPatch, addToHistory: boolean) => {
    const changedShapes: Record<string, TVShape | undefined> = {}
    const changedBindings: Record<string, TVBinding | undefined> = {}
    const changedAssets: Record<string, TVAsset | undefined> = {}
  ...
  onPatch = (state: TVSnapshot, patch: TelvaPatch, id?: string) => {
    if (
  ...
  onCommand = (state: TVSnapshot, command: TelvaCommand, id?: string) => {
  ...
  onPersist = (state: TVSnapshot, patch: TelvaPatch) => {
  ...
  getReservedContent = (coreReservedIds: string[], pageId = this.currentPageId) => {
    ...
    const reservedShapes: Record<string, TVShape> = {}
    const reservedBindings: Record<string, TVBinding> = {}
  ...
  public replacePageContent = (
    shapes: Record<string, TVShape>,
    bindings: Record<string, TVBinding>,
    assets: Record<string, TVAsset>,
    ...
    this.useStore.setState((current) => {
      ...
      Object.values(reservedShapes)
        ...
        .forEach((reservedShape) => {
          ...
          if (
            !(
              reservedShape.type === TVShapeType.Arrow ||
      ...
      // Get bindings related to the changed shapes
      const bindingsToUpdate = TVDR.getRelatedBindings(next, Object.keys(nextShapes), pageId)
      ...
      Object.values(nextShapes).forEach((shape) => {
        if (shape.type !== TVShapeType.Group) return
   ...
   */
  setNodeEditingId = (id?: string) => {
    this.patchState({ appState: { nodeEditingId: id } }, `set_node_editing_id`)
  }

  /**
   * Set a setting.
   */
  setSetting = <T extends keyof TVSnapshot['settings'], V extends TVSnapshot['settings'][T]>(
    name: T,
    value: V | ((value: V) => V)
  ): this => {
  ...
  toggleCanvasMode = (): this => {
    if (this.session) return this
    const nextMode = this.settings.canvasMode === 'freehand' ? 'straight' : 'freehand'
    const patch = { settings: { canvasMode: nextMode as 'freehand' | 'straight' } }
    this.patchState(patch, 'settings:toggled_canvas_mode')
    this.persist(patch)
    return this
  }

  get canvasMode() {
    return this.settings.canvasMode ?? 'freehand'
  }
  ...
  resetDocument = (): this => {
    ...
    const doc = TelvaApp.defaultDocument

    // Set the default page name to the localized version of "Page"
    doc.pages['page'].name = 'Page 1'

    this.resetHistory().clearSelectHistory().loadDocument(TelvaApp.defaultDocument).persist({})
   ...
   * @param document
   */
  updateUsers = (users: TVUser[], isOwnUpdate = false) => {
    this.patchState(
      {
        room: {
   ...
   * @param document
   */
  updateDocument = (document: TVDocument, reason = 'updated_document'): this => {
  ...
  loadDocument = (document: TVDocument): this => {
    ...
    this.replaceState(migrate(state, TelvaApp.version), 'loaded_document')
    const { point, zoom } = this.camera
    this.updateViewport(point, zoom)
   ...
   * @returns
   */
  loadPageFromURL = (page: TVPage, pageState: Record<string, TLPageState>) => {
    ...
    this.loadDocument(nextDocument as TVDocument)
    this.persist({})
  }
  ...
  saveProject = async () => {
    ...
    const fileHandle = await saveToFileSystem(
      migrate(this.state, TelvaApp.version).document,
   ...
   */
  saveProjectAs = async (filename?: string, format?: 'tldr' | 'tld') => {
    try {
      const fileHandle = await saveToFileSystem(this.document, null, filename, format)
      this.fileSystemHandle = fileHandle
      this.persist({})
      this.isDirty = false
    } catch (e: any) {
   ...
   */
  getPage = (pageId = this.currentPageId): TVPage => {
    return TVDR.getPage(this.state, pageId || this.currentPageId)
  }
   ...
   * @param pageId (optional) The page's id.
   */
  getShapes = (pageId = this.currentPageId): TVShape[] => {
    return TVDR.getShapes(this.state, pageId || this.currentPageId)
  }
   ...
   * @param pageId (optional) The page's id.
   */
  getBindings = (pageId = this.currentPageId): TVBinding[] => {
    return TVDR.getBindings(this.state, pageId || this.currentPageId)
  }
   ...
   * @param pageId (optional) The page's id.
   */
  getShape = <T extends TVShape = TVShape>(id: string, pageId = this.currentPageId): T => {
    return TVDR.getShape<T>(this.state, id, pageId)
  }
  ...
  getShapeBounds = (id: string, pageId = this.currentPageId): TLBounds => {
    return TVDR.getBounds(this.getShape(id, pageId))
  }
   ...
   * @param pageId (optional) The page's id.
   */
  getBinding = (id: string, pageId = this.currentPageId): TVBinding => {
    return TVDR.getBinding(this.state, id, pageId)
  }
  ...
  getPageState = (pageId = this.currentPageId): TLPageState => {
    return TVDR.getPageState(this.state, pageId || this.currentPageId)
  }
  ...
  copy = (ids = this.selectedIds, e?: ClipboardEvent): this => {
    ...
    const telvaString = `<telva>${jsonString}</telva>`

    setClipboard(telvaString)

    if (e) {
      e.clipboardData?.setData('text/html', telvaString)
    }

    if (navigator.clipboard && window.ClipboardItem) {
      navigator.clipboard.write([
        new ClipboardItem({
          'text/html': new Blob([telvaString], { type: 'text/html' }),
        }),
      ])
    }
  ...
  paste = async (point?: number[], e?: ClipboardEvent) => {
    ...
    const getSvgFromText = async (text: string) => {
      ...
      const imageBlob = await TVDR.getImageForSvg(svg, TVExportType.SVG, {
        scale: 1,
        quality: 1,
      })
    ...
    const getShapeFromText = (text: string) => {
      ...
      shapesToCreate.push(
        TVDR.getShapeUtil(TVShapeType.Text).getShape({
          id: Utils.uniqueId(),
          type: TVShapeType.Text,
          parentId: this.appState.currentPageId,
          text: TVDR.normalizeText(text.trim()),
          point: pagePoint,
          style: {
    ...
    const getShapeFromHtml = (html: string) => {
      try {
        const maybeJson = html.match(/<telva>(.*)<\/telva>/)?.[1]

        if (!maybeJson) return

        const json: {
          type: string
          shapes: (TVShape & { text: string })[]
          bindings: TVBinding[]
          assets: TVAsset[]
        } = JSON.parse(maybeJson)
        if (json.type === 'telva/clipboard') {
          clipboardData = json
          return
        } else {
          throw Error('Not telva data!')
        }
      } catch (e) {
  ...
  getSvg = async (
    ...
    if (opts.includeFonts) {
      try {
        const { fonts } = await fetch(TelvaApp.assetSrc, { mode: 'no-cors' }).then((d) => d.json())
        ...
        TVDR.warn('Could not find telva-assets.json file.')
      }
    } else {
    ...
    const commonBounds = Utils.getCommonBounds(shapes.map(TVDR.getRotatedBounds))

    // A quick routine to get an SVG element for each shape
    const getSvgElementForShape = async (shape: TVShape) => {
      const util = TVDR.getShapeUtil(shape)
      const bounds = util.getBounds(shape)
      const elm = await util.getSvgElement(shape, this.settings.isDarkMode)

      if (!elm) return

      // If the element is an image, set the asset src as the xlinkhref
      if (shape.type === TVShapeType.Image) {
        elm.setAttribute('href', this.document.assets[shape.assetId].src)
      } else if (shape.type === TVShapeType.Video) {
        elm.setAttribute('href', this.serializeVideo(shape.id))
      }
    ...
    for (const shape of shapes) {
      ...
      if (shape.children?.length) {
        ...
        for (const childId of shape.children) {
          const childShape = this.getShape(childId, this.currentPageId)
          const elm = await getSvgElementForShape(childShape)
      ...
      const elm = await getSvgElementForShape(shape)
    ...
    const exportBackground: TVExportBackground = this.settings.exportBackground
    const darkBackground = '#212529'
    const lightBackground = 'rgb(248, 249, 250)'

    switch (exportBackground) {
      case TVExportBackground.Auto: {
    ...
    svg
      .querySelectorAll('.tv-fill-hitarea, .tv-stroke-hitarea, .tv-binding-indicator')
  ...
  copySvg = async (
    ...
    const svgString = TVDR.getSvgString(svg, 1)

    this.clipboard = this.getContent(ids)

    const telvaString = JSON.stringify({
      type: 'telva/clipboard',
      shapes: [],
      assets: [],
      bindings: [],
  ...
  copyJson = (ids = this.selectedIds) => {
    ...
    if (content) {
      TVDR.copyStringToClipboard(JSON.stringify(content))
    }
   ...
   */
  insertContent = (
    content: { shapes: TVShape[]; bindings?: TVBinding[]; assets?: TVAsset[] },
   ...
   */
  getImage = async (
    format: Exclude<TVExportType, TVExportType.JSON> = TVExportType.PNG,
    opts = {} as Partial<{
      ids: string[]
      scale: number
      quality: number
    ...
    const svg = await this.getSvg(ids, {
      includeFonts: format !== TVExportType.SVG,
    })

    if (!svg) return

    if (format === TVExportType.SVG) {
      const svgString = TVDR.getSvgString(svg, 1)
      const blob = new Blob([svgString], { type: 'image/svg+xml' })
      return blob
    }

    const imageBlob = await TVDR.getImageForSvg(svg, format, opts)
   ...
   */
  copyImage = async (
    format: TVExportType.PNG | TVExportType.SVG = TVExportType.PNG,
    opts = {} as Partial<{
      ids: string[]
      scale: number
      quality: number
  ...
  convertToImage = async () => {
    ...
    const newShapes: TVShape[] = []
    const shapesToDelete: string[] = []

    for (const id of selectedIds) {
      const shape = this.getShape(id)
      if (shape.type !== TVShapeType.ReactComponent) continue

      const util = TVDR.getShapeUtil(shape) as any
      if (!util.getSvgElement) continue

      // Use the utility's getSvgElement which handles canvas/html2canvas capturing
      // We pass isDarkMode=false since we just want the raw component look
      const svgElm = await util.getSvgElement(shape, this.settings.isDarkMode)
      if (!svgElm) continue

      const href = svgElm.getAttribute('href')
      if (!href) continue

      // Create a new image shape with the dataUrl
      const imageShape = TVDR.getShapeUtil(TVShapeType.Image).create({
        id: Utils.uniqueId(),
        parentId: shape.parentId,
        childIndex: shape.childIndex,
        point: [...shape.point],
        size: [...(shape as any).size],
        style: { ...shape.style },
        assetId: 'asset-' + Utils.uniqueId(),
      })

      // Add the asset to the document
      this.patchState(
        {
          document: {
            assets: {
              [`${imageShape.assetId}`]: {
                id: imageShape.assetId,
                type: 'image',
                src: href,
                size: (shape as any).size,
                fileName: 'component-snapshot.png',
              } as any,
            },
          },
        },
      ...
      newShapes.push(imageShape)
      shapesToDelete.push(shape.id)
    }

    if (newShapes.length > 0) {
      this.createShapes(...newShapes)
      this.delete(shapesToDelete)
  ...
  zoomIn = (): this => {
    ...
    const nextZoom = TVDR.getCameraZoom((i + 1) * 0.25)
  ...
  zoomToFit = (): this => {
    ...
    const commonBounds = Utils.getCommonBounds(shapes.map(TVDR.getBounds))
    let zoom = TVDR.getCameraZoom(
      Math.min(
  ...
  zoomToSelection = (): this => {
    ...
    const { rendererBounds } = this
    const selectedBounds = TVDR.getSelectedBounds(this.state)
  ...
  zoomBy = Utils.throttle((delta: number, center?: number[]): this => {
    ...
    const nextZoom = TVDR.getCameraZoom(zoom - delta * zoom)
  ...
  startSession = <T extends SessionType>(type: T, ...args: SessionArgsOfType<T>): this => {
    ...
    if (this.session) {
      TVDR.warn(`Already in a session! (${this.session.constructor.name})`)
      this.cancelSession()
    }
   ...
   * @command
   */
  createShapes = (...shapes: ({ id: string; type: TVShapeType } & Partial<TVShape>)[]): this => {
    if (shapes.length === 0) return this

    return this.create(
      shapes.map((shape) => {
        return TVDR.getShapeUtil(shape.type).create({
   ...
   * @command
   */
  updateShapes = (...shapes: ({ id: string } & Partial<TVShape>)[]): this => {
  ...
  createTextShapeAtPoint(point: number[], id?: string, patch?: boolean): this {
    ...
    const Text = shapeUtils[TVShapeType.Text]
    ...
    if (patch) {
      this.patchCreate([TVDR.getShapeUtil(newShape.type).create(newShape)])
    } else {
  ...
  getImageOrVideoShapeAtPoint(
    id: string,
    type: TVShapeType.Image | TVShapeType.Video,
    point: number[],
    size: number[],
    assetId: string
  ) {
   ...
   */
  setShapeProps = <T extends TVShape>(props: Partial<T>, ids = this.selectedIds) => {
  ...
  group = (
    ...
    if (ids.length === 1 && this.getShape(ids[0], pageId).type === TVShapeType.Group) {
  ...
  ungroup = (ids = this.selectedIds, pageId = this.currentPageId): this => {
    ...
    const groups = ids
      ...
      .filter((shape) => shape.type === TVShapeType.Group)
  ...
  addMediaFromFiles = async (files: File[], point = this.centerPoint) => {
    ...
    for (const file of files) {
      ...
      const shapeType = isImage ? TVShapeType.Image : TVShapeType.Video
      const assetType = isImage ? TVAssetType.Image : TVAssetType.Video
    ...
    if (shapesToCreate.length) {
      ...
      const commonBounds = Utils.getCommonBounds(shapesToCreate.map(TVDR.getBounds))
  ...
  onZoom: TLWheelEventHandler = (info, e) => {
    if (this.state.appState.status !== TVStatus.Idle) return
  ...
  onPointerDown: TLPointerEventHandler = (info, e) => {
    ...
    if (this.currentTool.type === TVShapeType.Draw && e.pointerType === 'pen' && e.button === 5) {
  ...
  onPointerUp: TLPointerEventHandler = (info, e) => {
    ...
    if (this.isErasingWithPen && e.pointerType === 'pen' && e.button === 5) {
      this.selectTool(TVShapeType.Draw)
  ...
  getShapeUtil = TVDR.getShapeUtil
  ...
  static defaultState: TVSnapshot = {
    settings: {
      ...
      language: 'en',
      dockPosition: 'bottom',
      exportBackground: TVExportBackground.Transparent,
      canvasMode: 'freehand' as const,
    },
    appState: {
      status: TVStatus.Idle,
      activeTool: 'select',
    ...
    document: TelvaApp.defaultDocument,
  }

  static assetSrc = 'telva-assets.json'
}
