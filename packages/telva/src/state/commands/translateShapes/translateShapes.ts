...
import type { PagePartial, TelvaCommand } from '~types'

export function translateShapes(app: TelvaApp, ids: string[], delta: number[]): TelvaCommand {
  ...
  const bindingsToDelete = TVDR.getBindings(app.state, currentPageId).filter(
