...
import { ArrowShape, SessionType, TVShapeType, TVStatus } from '~types'

describe('Arrow session', () => {
  const restoreDoc = new TelvaTestApp()
  ...
  it('begins, updateSession', () => {
    ...
    expect(app.appState.status).toBe(TVStatus.Idle)
...
describe('When creating with an arrow session', () => {
  it('Deletes the shape on undo', () => {
    const app = new TelvaTestApp()
      .createShapes({ type: TVShapeType.Arrow, id: 'arrow1', point: [200, 200] })
      .select('arrow1')
      .movePointer([200, 200])
  ...
  it("Doesn't corrupt a shape after undoing", () => {
    const app = new TelvaTestApp().reset()

    expect(app.bindings.length).toBe(0)

    app
      .createShapes(
        { type: TVShapeType.Rectangle, id: 'rect1', point: [200, 200], size: [100, 100] },
        { type: TVShapeType.Rectangle, id: 'rect2', point: [400, 400], size: [100, 100] }
      )
      .selectTool(TVShapeType.Arrow)
  ...
  it('Creates a start binding if possible', () => {
    ...
    const arrow = app.shapes.find((shape) => shape.type === TVShapeType.Arrow) as ArrowShape
  ...
  it('Removes a binding when dragged away', () => {
    const app = new TelvaTestApp()
      .reset()
      .createShapes(
        { type: TVShapeType.Rectangle, id: 'rect1', point: [0, 0], size: [100, 100] },
        { type: TVShapeType.Arrow, id: 'arrow1', point: [200, 200] }
      )
...
describe('When creating arrows inside of other shapes...', () => {
  it('does not bind an arrow to shapes that contain the whole arrow', () => {
    ...
    expect(arrow.type).toBe(TVShapeType.Arrow)
    expect(app.bindings.length).toBe(0)
    expect(app.shapes.length).toBe(2)
  })
})
...
//       .selectAll()
//       .delete()
//       .createShapes({ type: TVShapeType.Arrow, id: 'arrow1', point: [0, 0] })
//       .select('arrow1')
//       .movePointer([0, 0])
//       .startSession(SessionType.Arrow, 'arrow1', 'start')
//       .movePointer([-10, -10])
