...
export class ArrowSession extends BaseSession {
  ...
  constructor(app: TelvaApp, shapeId: string, handleId: 'start' | 'end', isCreate = false) {
    super(app)
    ...
    this.bindableShapeIds = TVDR.getBindableShapeIds(app.state).filter(
    ...
    if (this.isCreate) {
      ...
      this.startBindingShapeId = this.bindableShapeIds
        ...
        .filter(
          (shape) =>
            !shape.isLocked &&
            Utils.pointInBounds(originPoint, TVDR.getShapeUtil(shape).getBounds(shape))
        )
        .sort((a, b) => {
  ...
  update = (): TelvaPatch | undefined => {
    ...
    const utils = shapeUtils[TVShapeType.Arrow]
    ...
    const next: { shape: ArrowShape; bindings: Record<string, TVBinding | undefined> } = {
    ...
    if (this.startBindingShapeId) {
      ...
      const startTargetUtils = TVDR.getShapeUtil(startTarget)
    ...
    if (!metaKey) {
      ...
      const targets = this.bindableShapeIds
        ...
        .filter((shape) => {
          ...
          const utils = TVDR.getShapeUtil(shape)
    ...
    const change = TVDR.getShapeUtil<ArrowShape>(next.shape).onHandleChange?.(
      next.shape,
      next.shape.handles
    )
  ...
  cancel = (): TelvaPatch | undefined => {
    ...
    const currentShape = TVDR.onSessionComplete(this.app.page.shapes[initialShape.id]) as ArrowShape

    const isDeleting =
      this.isCreate ||
      Vec.dist(currentShape.handles.start.point, currentShape.handles.end.point) < 4

    const afterBindings: Record<string, TVBinding | undefined> = {}
  ...
  complete = (): TelvaPatch | TelvaCommand | undefined => {
    ...
    const beforeBindings: Partial<Record<string, TVBinding>> = {}

    const afterBindings: Partial<Record<string, TVBinding>> = {}
  ...
  private findBindingPoint = (
    ...
    const util = TVDR.getShapeUtil<TVShape>(target.type)
