...
import { TLShapeUtil, Utils } from 'telva-core'
...
import { AlignStyle, ShapesWithProp, TVBinding, TVMeta, TVShape, TransformInfo } from '~types'
...
export abstract class TVShapeUtil<T extends TVShape, E extends Element = any> extends TLShapeUtil<
  T,
  E,
  TVMeta
> {
  ...
  updateChildren?: <K extends TVShape>(shape: T, children: K[]) => Partial<K>[] | void

  onChildrenChange?: (shape: T, children: TVShape[]) => Partial<T> | void
  ...
  getSvgElement = (
    shape: T,
    isDarkMode: boolean
  ): SVGElement | Promise<SVGElement | void> | void => {
    ...
    if (hasLabel) {
      const s = shape as TVShape & { label: string }
      ...
      const bounds = this.getBounds(shape)
      const wrapWidth = Math.max(0, bounds.width - 16) // 8px padding each side

      const labelSize = getTextLabelSize(shape.label!, font, wrapWidth)
      ...
      labelElm.setAttribute(
        'transform',
        `translate(${bounds.width / 2 - wrapWidth / 2}, ${bounds.height / 2 - labelSize[1] / 2})`
      )
