import { TLBounds, TLBoundsCorner, TLBoundsEdge, Utils } from 'telva-core'
import type { TLBoundsWithCenter, TLSnapLine } from 'telva-core'
...
export class TransformSession extends BaseSession {
  ...
  constructor(
    ...
    this.initialShapes = TVDR.getSelectedBranchSnapshot(
      this.app.state,
    ...
    this.isAllAspectRatioLocked = this.initialShapes.every(
      (shape) => shape.isAspectRatioLocked || TVDR.getShapeUtil(shape).isAspectRatioLocked
    )

    const shapesBounds = Object.fromEntries(
      this.initialShapes.map((shape) => [shape.id, TVDR.getBounds(shape)])
    )
  ...
  start = (): TelvaPatch | undefined => {
    this.snapInfo = {
      ...
      bounds: this.app.shapes
        ...
        .map((shape) => Utils.getBoundsWithCenter(TVDR.getRotatedBounds(shape))),
    }
  ...
  update = (): TelvaPatch | undefined => {
    ...
    const shapes = {} as Record<string, TVShape>
    ...
    shapeBounds.forEach(({ initialShape, initialShapeBounds, transformOrigin }) => {
      ...
      const afterShape = TVDR.transform(this.app.getShape(initialShape.id), newShapeBounds, {
  ...
  cancel = (): TelvaPatch | undefined => {
    ...
    const shapes = {} as Record<string, TVShape | undefined>
  ...
  complete = (): TelvaPatch | TelvaCommand | undefined => {
    ...
    const beforeShapes: Record<string, TVShape | undefined> = {}
    const afterShapes: Record<string, TVShape> = {}
