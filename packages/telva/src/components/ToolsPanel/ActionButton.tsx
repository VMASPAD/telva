...
import { AlignType, DistributeType, StretchType, TVSnapshot } from '~types'

const dockPositionState = (s: TVSnapshot) => s.settings.dockPosition

const selectedShapesCountSelector = (s: TVSnapshot) =>
  s.document.pageStates[s.appState.currentPageId].selectedIds.length

const isAllLockedSelector = (s: TVSnapshot) => {
  const page = s.document.pages[s.appState.currentPageId]
  const { selectedIds } = s.document.pageStates[s.appState.currentPageId]
  return selectedIds.every((id) => page.shapes[id].isLocked)
}

const isAllAspectLockedSelector = (s: TVSnapshot) => {
  const page = s.document.pages[s.appState.currentPageId]
  const { selectedIds } = s.document.pageStates[s.appState.currentPageId]
  return selectedIds.every((id) => page.shapes[id].isAspectRatioLocked)
}

const isAllGroupedSelector = (s: TVSnapshot) => {
  const page = s.document.pages[s.appState.currentPageId]
...
const hasSelectionSelector = (s: TVSnapshot) => {
  const { selectedIds } = s.document.pageStates[s.appState.currentPageId]
  return selectedIds.length > 0
}

const hasMultipleSelectionSelector = (s: TVSnapshot) => {
  const { selectedIds } = s.document.pageStates[s.appState.currentPageId]
