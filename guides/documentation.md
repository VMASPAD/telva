...
This file contains the documentation for the `<Telva>` component as well as the data model that the component accepts.

## `telva`

The `Telva` React component is the [telva](https://telva.com) editor exported as a standalone component. You can control the editor through props, or through the `TelvaApp`'s imperative API. **All props are optional.**

| Prop              | Type         | Description                                                                                               |
| ----------------- | ------------ | --------------------------------------------------------------------------------------------------------- |
| `id`              | `string`     | An id under which to persist the component's state.                                                       |
| `document`        | `TVDocument` | An initial [`TVDocument`](#TVDocument) object.                                                            |
| `currentPageId`   | `string`     | A current page id, referencing the `TVDocument` object provided via the `document` prop.                  |
...
| `onMount`         | `Function`   | Called when the editor first mounts, receiving the current `TelvaApp`.                                    |
...
| `onChange`        | `Function`   | Called when the `TelvaApp` updates for any reason.                                                        |
| `onUserChange`    | `Function`   | Called when the user's "presence" information changes.                                                    |
| `onUndo`          | `Function`   | Called when the `TelvaApp` updates after an undo.                                                         |
| `onRedo`          | `Function`   | Called when the `TelvaApp` updates after a redo.                                                          |
...
```ts
import { Telva, useFileSystem } from '@telva/telva'
...
You can initialize or control the `<Telva>` component via its `document` property. A `TVDocument` is an object with three properties:

- `id` - A unique ID for this document
- `pages` - A table of `TVPage` objects
- `pageStates` - A table of `TLPageState` objects
- `version` - The document's version, used internally for migrations.

```ts
import { TVDocument, TelvaApp } from '@telva/telva'

const myDocument: TVDocument = {
  id: 'doc',
...
function App() {
  return <Telva document={myDocument} />
}
```

**Tip:** The pages and pageStates in telva are objects containing `TLPage` and `TLPageState` objects from the [@telva/core](/packages/core) library.

**Tip:** The `assets` in telva is a table of `TVAssets` (images and videos).
...
Your `TLPage` objects may include shapes: objects that fit one of the `TelvaShape` interfaces listed below. All `TelvaShapes` extends a common interface:
...
| `handles`             | `TVHandle{}` | (optional) A table of `TLHandle` objects.                       |
...
You can change the `telva` component's state through an imperative API called `TelvaApp`. To access this API, use the `onMount` callback, or any of the component's callback props, like `onPersist`.

```tsx
import { Telva, TelvaApp } from '@telva/telva'
...
function App() {
  ...
  return <Telva onMount={handleMount} />
}
```
