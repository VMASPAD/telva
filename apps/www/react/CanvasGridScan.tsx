...
const INITIAL_CONTEXT_SIZE = 1080;
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    renderer.setSize(INITIAL_CONTEXT_SIZE, INITIAL_CONTEXT_SIZE, false);
    ...
    const syncToContainerSize = () => {
      const { width, height } = getMeasuredSize();

      // Keep the safe 1080x1080 context until the container has a real size.
      if (width <= 1 || height <= 1) {
        return;
      }
    ...
    requestAnimationFrame(syncToContainerSize);
    ...
    return () => {
      ...
      window.removeEventListener('resize', syncToContainerSize);
      material.dispose();
