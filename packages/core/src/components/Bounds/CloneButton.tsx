...
function _CloneButton({ bounds, side, targetSize, size }: CloneButtonProps) {
  ...
  return (
    <g className="tv-clone-target" transform={`translate(${x}, ${y})`} aria-label="clone button">
      <rect className="tv-transparent" width={targetSize * 2} height={targetSize * 2} />
      <g
        className="tv-clone-button-target"
        onPointerDown={handleClick}
        transform={`translate(${targetSize}, ${targetSize}) rotate(${ROTATIONS[side]})`}
      >
        <circle className="tv-transparent " r={targetSize} />
        <path
          className="tv-clone-button"
