...
import { GradientData, buildCssGradient } from '~types'
...
export const TextLabel = React.memo(function TextLabel({
  ...
  const handleChange = React.useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      onChange(TVDR.normalizeText(e.currentTarget.value))
    },
    [onChange]
  )
  ...
  React.useLayoutEffect(() => {
    ...
    if (!elm) return
    const size = getTextLabelSize(text, font, width)
    elm.style.transform = `scale(${scale}, ${scale}) translate(${offsetX}px, ${offsetY}px)`
    elm.style.width = width ? `${width}px` : size[0] + 1 + 'px'
    elm.style.height = size[1] + 1 + 'px'
  }, [text, font, offsetY, offsetX, scale, width])
  ...
  return (
    <TextWrapper>
      <InnerWrapper
        ...
        style={{
          font,
          color: textGradient && !isEditing ? 'transparent' : color,
          backgroundColor: background && background !== 'transparent' ? background : undefined,
          borderRadius: background && background !== 'transparent' ? 2 : undefined,
          padding: background && background !== 'transparent' ? '4px 6px' : undefined,
          ...(textGradient && !isEditing
            ? {
                background: buildCssGradient(textGradient),
