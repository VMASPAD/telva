...
import type { Patch, TVBinding, TelvaCommand } from '~types'
...
export function groupShapes(
  ...
  const shapesToGroup: TVShape[] = []
  const deletedGroupIds: string[] = []
  const otherEffectedGroups: TVShape[] = []
  ...
  const flattenedShapes = TVDR.flattenPage(app.state, pageId)
  ...
  const groupBounds = Utils.getCommonBounds(shapesToGroup.map((shape) => TVDR.getBounds(shape)))

  // Create the group
  beforeShapes[groupId] = undefined

  afterShapes[groupId] = TVDR.getShapeUtil(TVShapeType.Group).create({
    id: groupId,
