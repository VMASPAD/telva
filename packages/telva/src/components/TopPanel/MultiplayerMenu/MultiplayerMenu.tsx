...
import { TVAssetType, TVSnapshot } from '~types'

const roomSelector = (state: TVSnapshot) => state.room
...
export const MultiplayerMenu = function MultiplayerMenu() {
  ...
  const handleCopySelect = React.useCallback(() => {
    setCopied(true)
    TVDR.copyStringToClipboard(window.location.href)
  ...
  const handleCopyReadOnlySelect = React.useCallback(() => {
    ...
    TVDR.copyStringToClipboard(segs.join('/'))
    clearTimeout(rTimeout.current)
    rTimeout.current = setTimeout(() => setCopied(false), 1200)
  }, [])
  ...
  const handleCopyToMultiplayerProject = React.useCallback(async () => {
    ...
    try {
        ...
        for (const id in nextDocument.assets) {
          ...
          if (asset.src.includes('base64')) {
            const file = dataURLtoFile(
              asset.src,
              asset.fileName ?? asset.type === TVAssetType.Video ? 'image.png' : 'image.mp4'
            )
      ...
      if (response.ok && result?.url) {
        ...
        TVDR.warn(result?.message ?? response.statusText)
      }
    } catch (e) {
      TVDR.warn((e as any).message)
    }
