...
import type { PagePartial, TVAsset, TVBinding, TVShape, TelvaCommand } from '~types'
...
export function insertContent(
  ...
  const afterAssets: Record<string, TVAsset> = {}
  ...
  if (overwrite) {
    ...
    let nextIndex = TVDR.getTopChildIndex(app.state, currentPageId)

    const shapesToInsert: TVShape[] = content.shapes
    ...
    const shapesToMove = Object.values(after.shapes) as TVShape[]

    if (shapesToMove.length > 0) {
      if (point) {
        // Move the shapes so that they're centered on the given point
        const commonBounds = Utils.getCommonBounds(
          shapesToMove.map((shape) => TVDR.getBounds(shape))
        )
        ...
        const commonBounds = Utils.getCommonBounds(shapesToMove.map(TVDR.getBounds))
  ...
  Object.values(after.shapes as Record<string, TVShape>).forEach((shape) => {
    if ('text' in shape) {
