...
- Adds the `isGhost` prop to `TLShape`. In `TLComponentProps`, the `isGhost` prop will be true if either a shape has its `isGhost` set to `true` OR if a shape is the descendant of a shape with `isGhost` set to `true`. A ghost shape will have the `tv-ghost` class name, though this is not used in the Renderer. You can set it yourself in your app.
...
- Expands README.
- Removes properties specific to the telva app.

## 0.1.7

- Fixes selection bug with SVGContainer.
- Removes various properties specific to the telva app.
