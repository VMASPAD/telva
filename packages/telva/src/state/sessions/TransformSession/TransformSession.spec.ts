import { TLBoundsCorner, TLBoundsEdge, Utils } from 'telva-core'
import { TVDR } from '~state/TVDR'
import { TelvaTestApp, mockDocument } from '~test'
import { TVShapeType, TVStatus } from '~types'

function getShapeBounds(app: TelvaTestApp, ...ids: string[]) {
  return Utils.getCommonBounds(
    (ids.length ? ids : app.selectedIds).map((id) => TVDR.getBounds(app.getShape(id)))
  )
}
...
describe('When holding alt', () => {
  it('resizes edge from center', () => {
    ...
    const beforeCenter = Utils.getBoundsCenter(
      Utils.getCommonBounds(app.shapes.map(TVDR.getBounds))
    )
    ...
    const afterCenter = Utils.getBoundsCenter(Utils.getCommonBounds(app.shapes.map(TVDR.getBounds)))
