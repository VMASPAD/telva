import * as React from 'react'
import { Container } from '~components/Container'
import { SVGContainer } from '~components/SVGContainer'
import { useHandleEvents } from '~hooks'
import Utils from '~utils'

interface HandleProps {
  id: string
  point: number[]
}

function _Handle({ id, point }: HandleProps) {
  const events = useHandleEvents(id)

  return (
    <Container
      bounds={Utils.translateBounds(
        {
          minX: 0,
          minY: 0,
          maxX: 0,
          maxY: 0,
          width: 0,
          height: 0,
        },
        point
      )}
    >
      <SVGContainer>
        <g className="tv-handle" aria-label="handle" {...events}>
          <circle className="tv-handle-bg" pointerEvents="all" />
          <circle className="tv-counter-scaled tv-handle" pointerEvents="none" r={4} />
        </g>
      </SVGContainer>
    </Container>
  )
}

export const Handle = React.memo(_Handle)
