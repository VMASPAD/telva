import { TLBounds, TLPageState, TLTransformInfo, Utils } from 'telva-core'
import { intersectRayBounds, intersectRayEllipse, intersectRayLineSegment } from 'telva-intersect'
...
export class TVDR {
  ...
  static getShapeUtil<T extends TVShape>(shape: T): TVShapeUtil<T>
  static getShapeUtil<T extends TVShape>(shape: T | T['type']) {
    return getShapeUtil<T>(shape)
  }

  static getSelectedShapes(data: TVSnapshot, pageId: string) {
    const page = TVDR.getPage(data, pageId)
    const selectedIds = TVDR.getSelectedIds(data, pageId)
    return selectedIds.map((id) => page.shapes[id])
  }

  static screenToWorld(data: TVSnapshot, point: number[]) {
    const camera = TVDR.getPageState(data, data.appState.currentPageId).camera
  ...
  static getPage(data: TVSnapshot, pageId: string): TVPage {
    return data.document.pages[pageId]
  }

  static getPageState(data: TVSnapshot, pageId: string): TLPageState {
    return data.document.pageStates[pageId]
  }

  static getSelectedIds(data: TVSnapshot, pageId: string): string[] {
    return TVDR.getPageState(data, pageId).selectedIds
  }

  static getShapes(data: TVSnapshot, pageId: string): TVShape[] {
    return Object.values(TVDR.getPage(data, pageId).shapes)
  }

  static getCamera(data: TVSnapshot, pageId: string): TLPageState['camera'] {
    return TVDR.getPageState(data, pageId).camera
  }

  static getShape<T extends TVShape = TVShape>(
    data: TVSnapshot,
    shapeId: string,
    pageId: string
  ): T {
    return TVDR.getPage(data, pageId).shapes[shapeId] as T
  }

  static getCenter<T extends TVShape>(shape: T) {
    return TVDR.getShapeUtil(shape).getCenter(shape)
  }

  static getBounds<T extends TVShape>(shape: T) {
    return TVDR.getShapeUtil(shape).getBounds(shape)
  }

  static getRotatedBounds<T extends TVShape>(shape: T) {
    return TVDR.getShapeUtil(shape).getRotatedBounds(shape)
  }

  static getSelectedBounds(data: TVSnapshot): TLBounds {
    return Utils.getCommonBounds(
      TVDR.getSelectedShapes(data, data.appState.currentPageId).map((shape) =>
        TVDR.getShapeUtil(shape).getBounds(shape)
      )
    )
  }

  static getParentId(data: TVSnapshot, id: string, pageId: string) {
    return TVDR.getShape(data, id, pageId).parentId
  }

  // static getPointedId(data: TVSnapshot, id: string, pageId: string): string {
  //   const page = TVDR.getPage(data, pageId)
  //   const pageState = TVDR.getPageState(data, data.appState.currentPageId)
  //   const shape = TVDR.getShape(data, id, pageId)
  //   if (!shape) return id

  //   return shape.parentId === pageState.currentParentId || shape.parentId === page.id
  //     ? id
  //     : TVDR.getPointedId(data, shape.parentId, pageId)
  // }

  // static getDrilledPointedId(data: TVSnapshot, id: string, pageId: string): string {
  //   const shape = TVDR.getShape(data, id, pageId)
  //   const { currentPageId } = data.appState
  //   const { currentParentId, pointedId } = TVDR.getPageState(data, data.appState.currentPageId)

  //   return shape.parentId === currentPageId ||
  //     shape.parentId === pointedId ||
  //     shape.parentId === currentParentId
  //     ? id
  //     : TVDR.getDrilledPointedId(data, shape.parentId, pageId)
  // }

  // static getTopParentId(data: TVSnapshot, id: string, pageId: string): string {
  //   const page = TVDR.getPage(data, pageId)
  //   const pageState = TVDR.getPageState(data, pageId)
  //   const shape = TVDR.getShape(data, id, pageId)
  ...
  //     ? id
  //     : TVDR.getTopParentId(data, shape.parentId, pageId)
  // }

  // Get an array of a shape id and its descendant shapes' ids
  static getDocumentBranch(data: TVSnapshot, id: string, pageId: string): string[] {
    const shape = TVDR.getShape(data, id, pageId)
    ...
    return [
      id,
      ...shape.children.flatMap((childId) => TVDR.getDocumentBranch(data, childId, pageId)),
    ]
  }
  ...
  static getSelectedBranchSnapshot(data: TVSnapshot, pageId: string): TVShape[]
  static getSelectedBranchSnapshot<K>(
    data: TVSnapshot,
    pageId: string,
    fn?: (shape: TVShape) => K
    ...
    const copies = TVDR.getSelectedIds(data, pageId)
      .flatMap((id) => TVDR.getDocumentBranch(data, id, pageId).map((id) => page.shapes[id]))
  ...
  static getSelectedShapeSnapshot(data: TVSnapshot, pageId: string): TVShape[]
  ...
  static getSelectedShapeSnapshot<K>(
    ...
    const copies = TVDR.getSelectedShapes(data, pageId)
      .filter((shape) => !shape.isLocked)
      .map(Utils.deepClone)
  ...
  static getAllEffectedShapeIds(data: TVSnapshot, ids: string[], pageId: string): string[] {
    const page = TVDR.getPage(data, pageId)
    ...
    ids.forEach((id) => {
      ...
      function collectDescendants(shape: TVShape): void {
      ...
      function collectAscendants(shape: TVShape): void {
  ...
  static getChildIndexAbove(data: TVSnapshot, id: string, pageId: string): number {
    const page = data.document.pages[pageId]
    const shape = page.shapes[id]

    let siblings: TVShape[]
  ...
  static getBeforeShape<T extends TVShape>(shape: T, change: Partial<T>): Partial<T> {
    return Object.fromEntries(
      Object.keys(change).map((k) => [k, shape[k as keyof T]])
    ) as Partial<T>
  }

  static mutateShapes<T extends TVShape>(
    data: TVSnapshot,
    ids: string[],
    ...
    ids.forEach((id, i) => {
      const shape = TVDR.getShape<T>(data, id, pageId)
      ...
      if (shape?.type === 'group' && (ids.length === 1 || forceChildrenTraversal)) {
        shape.children.forEach((id, i) => {
          ...
          if (change) {
            beforeShapes[id] = TVDR.getBeforeShape(shape, change)
  ...
  static createShapes(data: TVSnapshot, shapes: TVShape[], pageId: string): TelvaCommand {
    const before: TelvaPatch = {
      document: {
        pages: {
          [pageId]: {
            shapes: {
              ...Object.fromEntries(
                shapes.flatMap((shape) => {
                  const results: [string, Partial<TVShape> | undefined][] = [[shape.id, undefined]]

                  // If the shape is a child of another shape, also save that shape
                  if (shape.parentId !== pageId) {
                    const parent = TVDR.getShape(data, shape.parentId, pageId)
                    if (!parent.children) throw Error('No children in parent!')
          ...
          [pageId]: {
            shapes: {
              shapes: {
                ...Object.fromEntries(
                  shapes.flatMap((shape) => {
                    const results: [string, Partial<TVShape> | undefined][] = [[shape.id, shape]]
  ...
  static deleteShapes(
    ...
    const shapeIds =
      typeof shapes[0] === 'string'
        ? (shapes as string[])
        : (shapes as TVShape[]).map((shape) => shape.id)
  ...
  static onSessionComplete<T extends TVShape>(shape: T) {
    const delta = TVDR.getShapeUtil(shape).onSessionComplete?.(shape)
    if (!delta) return shape
    return { ...shape, ...delta }
  }

  static onChildrenChange<T extends TVShape>(data: TVSnapshot, shape: T, pageId: string) {
    if (!shape.children) return

    const delta = TVDR.getShapeUtil(shape).onChildrenChange?.(
      shape,
      shape.children.map((id) => TVDR.getShape(data, id, pageId))
    )
  ...
  static updateArrowBindings(page: TVPage, arrowShape: ArrowShape) {
    const result = {
    ...
    if (arrowShape.handles.start.bindingId) {
      ...
      const util = TVDR.getShapeUtil(target)
          ...
          switch (A.target.type) {
            ...
            default: {
              ...
              if (
                ...
                return TVDR.getShapeUtil<ArrowShape>(arrowShape).onHandleChange?.(arrowShape, {
                  [ID]: {
    ...
    return TVDR.getShapeUtil<ArrowShape>(arrowShape).onHandleChange?.(arrowShape, result)
  }

  static transform<T extends TVShape>(shape: T, bounds: TLBounds, info: TLTransformInfo<T>) {
    const delta = TVDR.getShapeUtil(shape).transform(shape, bounds, info)
    if (!delta) return shape
    return { ...shape, ...delta }
  }

  static transformSingle<T extends TVShape>(shape: T, bounds: TLBounds, info: TLTransformInfo<T>) {
    const delta = TVDR.getShapeUtil(shape).transformSingle(shape, bounds, info)
    if (!delta) return shape
    return { ...shape, ...delta }
  }
   ...
   */
  static getRotatedShapeMutation<T extends TVShape>(
  ...
  static updateParents(data: TVSnapshot, pageId: string, changedShapeIds: string[]): void {
    const page = TVDR.getPage(data, pageId)

    if (changedShapeIds.length === 0) return

    const { shapes } = TVDR.getPage(data, pageId)
    ...
    for (const parentId of parentToUpdateIds) {
      ...
      TVDR.onChildrenChange(data, parent, pageId)
    }

    TVDR.updateParents(data, pageId, parentToUpdateIds)
  }
  ...
  static getBinding(data: TVSnapshot, id: string, pageId: string): TVBinding {
    return TVDR.getPage(data, pageId).bindings[id]
  }

  static getBindings(data: TVSnapshot, pageId: string): TVBinding[] {
    const page = TVDR.getPage(data, pageId)
    return Object.values(page.bindings)
  }

  static getBindableShapeIds(data: TVSnapshot) {
    return TVDR.getShapes(data, data.appState.currentPageId)
      .filter((shape) => TVDR.getShapeUtil(shape).canBind)
      .sort((a, b) => b.childIndex - a.childIndex)
      .map((shape) => shape.id)
  }

  static getBindingsWithShapeIds(data: TVSnapshot, ids: string[], pageId: string): TVBinding[] {
    return Array.from(
      new Set(
        TVDR.getBindings(data, pageId).filter((binding) => {
  ...
  static getRelatedBindings(data: TVSnapshot, ids: string[], pageId: string): TVBinding[] {
  ...
  static flattenShape = (data: TVSnapshot, shape: TVShape): TVShape[] => {
    return [
      shape,
      ...(shape.children ?? [])
        .map((childId) => TVDR.getShape(data, childId, data.appState.currentPageId))
        .sort((a, b) => a.childIndex - b.childIndex)
        .flatMap((shape) => TVDR.flattenShape(data, shape)),
    ]
  }

  static flattenPage = (data: TVSnapshot, pageId: string): TVShape[] => {
    return Object.values(data.document.pages[pageId].shapes)
      .sort((a, b) => a.childIndex - b.childIndex)
      .reduce<TVShape[]>((acc, shape) => [...acc, ...TVDR.flattenShape(data, shape)], [])
  }

  static getTopChildIndex = (data: TVSnapshot, pageId: string): number => {
    const shapes = TVDR.getShapes(data, pageId)
  ...
  static assertShapeHasProperty<P extends keyof TVShape>(
    shape: TVShape,
    prop: P
  ...
  static getSvgAsDataUrl(svg: SVGElement, scale = 1) {
    const svgString = TVDR.getSvgString(svg, scale)
  ...
  static async getImageForSvg(
    svg: SVGElement,
    type: Exclude<TVExportType, TVExportType.JSON> = TVExportType.PNG,
    opts = {} as Partial<{
      scale: number
      quality: number
    }>
